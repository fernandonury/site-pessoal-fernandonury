if (window.matchMedia("(max-width: 320px)").matches) {
    
    document.getElementById('tit00').style.marginTop = '15px'
    document.getElementById('txt00').style.marginTop = '15px'

    document.getElementById('cupido').style.display = 'none'

    document.getElementById('fernandonury01').style.top = '960px'

    document.getElementById('txt01').style.top = '1160px'

    document.getElementById('tit01').style.top = '1770px'

    document.getElementById('logo-jocum').style.top = '1787.5px'
    document.getElementById('logo-jocum').style.marginLeft = '67%'

}

if (window.matchMedia("(min-width: 360px)").matches) {

    document.getElementById('fernandonury01').style.top = '907px'

    document.getElementById('txt01').style.top = '1130px'
    
    document.getElementById('tit01').style.top = '1685px'

    document.getElementById('logo-jocum').style.top = '1703px'
    document.getElementById('logo-jocum').style.marginLeft = '61%'

    document.getElementById('txt01').style.top = '1130px'

    document.getElementById('img00').style.top = '2190px'
    document.getElementById('img01').style.top = '2395px'
    document.getElementById('img02').style.top = '2600px'

    document.getElementById('tit02').style.marginTop = '2295px'

    document.getElementById('txt04').style.top = '3230px'
    document.getElementById('txt05').style.top = '3295px'

}

if (window.matchMedia("(min-width: 375px)").matches) {

    document.getElementById('fernandonury01').style.top = '890px'

    document.getElementById('txt01').style.top = '1120px'
    
    document.getElementById('tit01').style.top = '1665px'

    document.getElementById('logo-jocum').style.top = '1685px'
    document.getElementById('logo-jocum').style.marginLeft = '59%'

    document.getElementById('txt02').style.top = '1720px'

    document.getElementById('img00').style.top = '2180px'
    document.getElementById('img01').style.top = '2385px'
    document.getElementById('img02').style.top = '2590px'

    document.getElementById('txt03').style.top = '2780px'

    document.getElementById('tit02').style.marginTop = '2280px'

    document.getElementById('txt04').style.top = '3200px'
    document.getElementById('txt05').style.top = '3265px'

}

if (window.matchMedia("(min-width: 414px)").matches) {

    document.getElementById('fernandonury01').style.top = '835px'

    document.getElementById('txt01').style.top = '1110px'
    
    document.getElementById('tit01').style.top = '1565px'

    document.getElementById('logo-jocum').style.top = '1582px'
    document.getElementById('logo-jocum').style.marginLeft = '55%'

    document.getElementById('txt02').style.top = '1620px'

    document.getElementById('img00').style.top = '2020px'
    document.getElementById('img01').style.top = '2225px'
    document.getElementById('img02').style.top = '2430px'

    document.getElementById('txt03').style.top = '2625px'

    document.getElementById('tit02').style.marginTop = '2150px'

    document.getElementById('txt04').style.top = '3000px'
    document.getElementById('txt05').style.top = '3050px'

}